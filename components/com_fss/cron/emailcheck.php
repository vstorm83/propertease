...
class FSSCronEMailCheck extends FSSCron
{
	...
	function Execute($aparams)
	{
		...
		foreach ($mails as $i)
		{
			...
			// This is not needed any more as we are only listed unread mails
			/*if ($aparams['type'] == "imap" && $this->IsMessageRead())
			{
				$this->EmailResult("Skipping read message", FSS_EMR_SKIP_READ);
				continue;
			}*/
			...
			if ($ticketid > 0)
			{
				...
				if ($this->params['allowunknown'] < 1)
				{
					...
					if ($userid > 0 && $userid != $ticket['handler_id'] && $userid != $ticket['user_id'])
					{
						$this->EmailResult("Unknown user replying to the message, ignore the email ({$this->headers->from[0]->mailbox}@{$this->headers->from[0]->host})", FSS_EMR_SKIP_UNKNOWNEMAIL);	
						continue;
					}
				}
			}
			...
			if ($ticketid > 0)
			{
				...
				if ($ticket['user_id'] == 0)
				{
					...
					} else if ($userid > 0 && $userid == $ticket['user_id']) {
	...
	function GetHeaders($i)
	{
		...
		$parsed = $this->parseFullHeaders(imap_fetchheader($this->conn, $i));
		...
		foreach ($headers as $header => $value)
		{
			if (is_string($value))
			{
				...
				$obj = new stdClass();
				$obj->text = iconv_mime_decode($value,0,"UTF-8");
				$obj->charset = 'UTF-8';
		...
		if (array_key_exists("X-Google-Original-From", $parsed))
			$this->headers->from = imap_rfc822_parse_adrlist($parsed["X-Google-Original-From"], '');
		
		if (array_key_exists("X-Original-From", $parsed))
			$this->headers->from = imap_rfc822_parse_adrlist($parsed["X-Original-From"], '');

...
class FssViewAdmin_Support_List extends FssViewAdmin_Support
{
	...
	function listRow(&$row)
	{
		...
		FSSParserTicket::forAdmin($this->parser,$row, $this->customfields);
		$this->setupParserView();
		...
		if ($this->merge)
		{
			...
			{
				$link = FSSRoute::_('index.php?option=com_fss&view=admin_support&task=merge.related&source_id=' . $session->get('merge_ticket_id') . '&dest_id=' . $row->id, false);
				if ($row->id != $session->get('merge_ticket_id'))
					$this->parser->SetVar('mergebutton', "<a href='$link' class='btn btn-default btn-small'>" . JText::_('ADD_RELATED') . "</a>");
			} else {
				if ($this->merge == "into")
				{
					$link = FSSRoute::_('index.php?option=com_fss&view=admin_support&task=merge.merge&source_id=' . $session->get('merge_ticket_id') . '&dest_id=' . $row->id, false);
				} else {
					$link = FSSRoute::_('index.php?option=com_fss&view=admin_support&task=merge.merge&source_id=' . $row->id . '&dest_id=' . $session->get('merge_ticket_id'), false);
				}
			
				if ($row->id != $session->get('merge_ticket_id') && $row->merged == 0)
				{

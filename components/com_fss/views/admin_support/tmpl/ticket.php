...
function addCC(user_id, is_admin, is_readonly)
{
	...
	var url = "<?php echo FSSRoute::x("&ticketid=" . $this->ticket->id . "&task=ticket.addcc", false); ?>&ids=" + escape(ids) + "&is_readonly=" + escape(is_readonly) + "&is_admin=" + escape(is_admin);
...
{
	var url = "<?php echo FSSRoute::x("&ticketid=" . $this->ticket->id . "&task=ticket.addemailcc", false); ?>&email=" + escape(email);
	window.location = url;
}

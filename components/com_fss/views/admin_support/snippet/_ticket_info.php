		...
		<td>
			...
			<?php foreach($this->ticket->user_cc as $cc): ?>
				<div class="fss_tag label user_cc label-small-close  <?php echo $cc->readonly || $cc->uremail ? 'label-warning' : 'label-success'; ?>" id="user_cc_<?php echo $cc->id; ?>">
					<a class="close" href="<?php echo FSSRoute::x("&ticketid=" . $this->ticket->id . "&task=ticket.removecc&is_admin=0&ids=" . $cc->id, false); ?>">&times;</a>
...
<?php 
	$userticketcount = SupportHelper::getUserTicketCounts($this->ticket->user_id,$this->ticket->email);
	...
	<td>
		<?php if ($this->can_ChangeTicket()): ?>
			<form id='status_form' action="<?php echo FSSRoute::_( 'index.php?option=com_fss&view=admin_support&task=update.ticket_status_id&ticketid=' . $this->ticket->id ); ?>" method="post" style="margin: 0px;">
				...
				<select id='ticket_status_id' name='ticket_status_id' class="input-medium select-color" style="margin: 0px; color: <?php echo FSS_Helper::escape($this->ticket->scolor); ?>" onchange="jQuery('#status_form').submit();">
		...
		<?php else: ?>
			<span style='color: <?php echo FSS_Helper::escape($this->ticket->scolor); ?>'>
...
<?php if (FSS_Settings::get('support_hide_priority') != 1) : ?>
	<?php FSS_Table::ColStart("tr_pri"); ?>
		...
		<td>
			<?php if ($this->can_ChangeTicket()): ?>
				<select id='ticket_pri_id' name='ticket_pri_id' class="input-medium select-color" style="margin: 0px; color:<?php echo FSS_Helper::escape($this->ticket->pcolor); ?>" onchange="priority_update();">

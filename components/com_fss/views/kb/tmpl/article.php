...
<?php if ($this->kb_rate): ?>
	...
	ev.preventDefault();
		jQuery.get('<?php echo JRoute::_( '&tmpl=component&rate=up&kbartid=', false); ?>' + '&kbartid=<?php echo $this->art['id']; ?>');
		jQuery('#fss_kb_rate').html("");
		jQuery('<div/>', {class: 'fss_kb_rate_head', text: '<?php echo JText::_("THANK_YOU_FOR_YOUR_FEEDBACK"); ?>'}).appendTo('#fss_kb_rate');
	});	

	jQuery('#rate_same').click( function (ev) {
		ev.preventDefault();
		jQuery.get('<?php echo JRoute::_( '&tmpl=component&rate=same&kbartid=' , false); ?>' + '&kbartid=<?php echo $this->art['id']; ?>');
		jQuery('#fss_kb_rate').html("");
		jQuery('<div/>', {class: 'fss_kb_rate_head', text: '<?php echo JText::_("THANK_YOU_FOR_YOUR_FEEDBACK"); ?>'}).appendTo('#fss_kb_rate');
	});
	...
	jQuery('#rate_down').click( function (ev) {
		ev.preventDefault();
		jQuery.get('<?php echo JRoute::_( '&tmpl=component&rate=down&kbartid=', false); ?>' + '&kbartid=<?php echo $this->art['id']; ?>');
		jQuery('#fss_kb_rate').html("");
		jQuery('<div/>', {class: 'fss_kb_rate_head', text: '<?php echo JText::_("THANK_YOU_FOR_YOUR_FEEDBACK"); ?>'}).appendTo('#fss_kb_rate');
	});
});

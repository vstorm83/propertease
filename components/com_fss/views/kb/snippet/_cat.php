...
<?php if (  (array_key_exists("subcats",$cat) && count($cat['subcats']) > 0) || (array_key_exists("arts",$cat) && count($cat['arts'])) || $this->view_mode_cat == "normal" || FSS_Input::getInt('catid') == $cat['id'] ): ?>
	...
	if ($can_show): ?>
		<div class='media' >
			<?php if ($cat['image']) : ?>
			<a class='pull-left' href="<?php if ($this->view_mode_cat == "accordian"): ?>#<?php else: ?><?php echo FSSRoute::_( '&what=&limitstart=&catid=' . $cat['id'] );// FIX LINK?><?php endif; ?>" onclick="return false;"
			...
			<div class="media-body">
					...
					<div 
						...
						<h4 class='media-heading' <?php if ($this->view_mode_cat == "accordian") echo " style='padding-top:6px;padding-bottom:6px;' "; ?>>
							...
							<?php if ($this->view_mode_cat == "popup") : ?>
								...
								<a class="show_modal_iframe" href='<?php echo FSSRoute::_( '&what=&tmpl=component&limitstart=catid=' . $cat['id'] . '&view_mode=' . $this->view_mode_incat );// FIX LINK ?>'>
									<?php echo $cat['title'] ?>
								</a>
							...
							<?php else: ?>
								...
								<?php if ($this->view_mode_cat == "normal"): ?>
									<a href='<?php echo FSSRoute::_( '&what=&limitstart=&catid=' . $cat['id'] );// FIX LINK?>'><?php echo $cat['title'] ?></a>
								<?php else: ?>
									<?php echo $cat['title'] ?>
								<?php endif; ?>
				...
				<div class="fss_clear"></div>
					<div id="kb_cat_content_<?php echo $cat['id']; ?>" class="collapse" <?php if (!$cat['image']) : ?> style="margin-left: 16px;" <?php endif; ?>>
						<!-- Category contents -->

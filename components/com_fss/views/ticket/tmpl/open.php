...
<?php if (FSS_Settings::get('support_sel_prod_dept') && ($this->product['title'] || $this->dept['title'])): ?>
	...
	<div class="form-horizontal form-condensed">
		<?php if ($this->product['title']): ?>
			<?php FSS_Translate_Helper::TrSingle($this->product); ?>
		...
		<?php if ($this->dept['title']): ?>
			<?php FSS_Translate_Helper::TrSingle($this->dept); ?>
...
<?php $has_message_details = false; ?>
	...
	<div class="form-horizontal form-condensed">
		...
		<?php if (!FSS_Settings::get('support_hide_tags') && $this->admin_create > 0): ?>
			<div class="control-group cg-tags <?php echo $this->errors['subject'] ? 'error' : ''; ?>">
				<?php 
		...
		<div class="controls">
			...
			<select id='catid' class='input-large' name='catid' <?php if (FSS_Settings::get('support_altcat')) echo "required" ?>>
				...
				<?php foreach ($this->cats as $cat): ?>
					...
					?>
					<option value='<?php echo $cat['id']; ?>' <?php if (FSS_Input::GetInt('catid') == $cat['id']) echo "selected='selected'"; ?>><?php echo $cat['title']; ?></option>
				<?php endforeach; ?>
					...
					<?php if ($this->admin_create > 0): ?>
						...
						<optgroup label="<?php echo JText::_("ASSIGNED_HANDLERS"); ?>">
							<?php foreach ($this->handlers as $handler): ?>
								...
								<?php if ($handler['id'] == $this->ticket['handler'] && $this->ticket['handler'] > 0) echo "selected='selected'"; ?>>

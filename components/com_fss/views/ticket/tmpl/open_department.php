...
<?php if ($this->product && FSS_Settings::get('support_sel_prod_dept')): ?>
...
<?php if (FSS_Settings::get('support_advanced_department')): ?>
	...
	<form id="searchDept" action="<?php echo FSSRoute::_( 'index.php?limitstart=0&option=com_fss&layout=open&view=ticket' );?>" method="post" name="fssForm"
...
<form name='deptselect' action='<?php echo FSSRoute::_('&layout=open');// FIX LINK?>' method='post'>
	<p>
		...
		<?php if (!FSS_Settings::get('support_next_prod_click')): ?>
			<input class='btn btn-primary' type='submit' id='pickdept' value='<?php echo JText::_("NEXT"); ?>'>
		<?php endif; ?>
	</p>

	<?php FSS_Translate_Helper::Tr($this->depts); ?>

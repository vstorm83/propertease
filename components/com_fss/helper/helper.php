...
class FSS_Helper
{
	...
	static function StylesAndJS($params = array(), $css = array(), $js = array())
	{
		...
		if (!self::$styles_incl)
		{
			...
			if (FSS_Settings::get('bootstrap_v3'))
			{
	...
	static function GetRouteMenus()
	{
		...
		if (empty($FSSRoute_menus))
		{
			$FSSRoute_menus = array();
			$db = JFactory::getDBO();
			$qry = "SELECT id, link, access FROM #__menu WHERE link LIKE '%option=com_fss%' AND published = 1";			
	...
	static function PageStyle()
	{
		...
		$output = "<div class='fss_main $class'>\n";
...
<?php		
	...
	{
		$html[] = '</div>';
		$html[] = '<div class="modal-footer fss_main">';
	...
	{		
		...
		$html[] = '<div class="modal-header fss_main">';
		...
		$html[] = '</h3>';
		$html[] = '</div>';
		$html[] = '<div class="modal-body fss_main">';
	...
	{  
		...
		{
			if (FSS_Settings::get('support_attach_max_size_admins') > 0)
				return FSS_Settings::get('support_attach_max_size_admins');
		} else {
			if (FSS_Settings::get('support_attach_max_size') > 0)
				return FSS_Settings::get('support_attach_max_size');
		}
	...
	static function IsPluignEnabled($type, $plugin)
	{
		if (empty(self::$plugin_data))
		{
			...
			foreach ($pluigns as $plg)
			{
				...
				self::$plugin_data[$key] = $plg->enabled;
			}
		}

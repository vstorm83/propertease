...
// the data model handles initialisation of the form and validation of input

class FlexicontactplusModelData extends LAFP_model
{
var $_data;
var $_app = null;
...
// user defined fields
	...
	for ($field_index=0; $field_index < $num_fields; $field_index++) 
		...
		switch ($field->field_type)
			...
			default:
				$this->data->$field_id = $jinput->get($field_id, $field->default_value, 'STRING');
			}
...
// Now sort out the Post data default, if it exists
		...
		if (preg_match($regex, $field->default_value, $matches) >0 )
			{
			$this->data->$field_id = htmlspecialchars($jinput->get($matches[1], '', 'STRING'), ENT_COMPAT, 'UTF-8');			
			$value = $matches[1];
...
// $this->data ends up being an Array
//
function get_data($config_data)
{
FCP_trace::trace("** Data model get_data()");
	$this->data = array_merge($_GET, $_POST);			// get both as a single array
	foreach ($this->data as &$item)
		$item = rawurldecode(rawurldecode($item));
		
// if this is a form send, we need to create entries for unchecked check boxes so that the mandatory check works

	$task = $this->data['task'];
	if (substr($task,0,4) != 'send')					// check for send1 or send2
		return;
	...
	foreach ($config_data->all_fields as $field_index => $field)
		...
		switch ($field->field_type)
			...
			case LAFC_FIELD_CHECKBOX_L:
			case LAFC_FIELD_CHECKBOX_R:
			case LAFC_FIELD_CHECKBOX_H:									// CHECKBOX_M cannot (currently) be mandatory 
	...
	foreach ($this->data as $field_id => $field_value)
		...
		if (FCP_trace::tracing())
			...
			if (strlen($field_id) == 8)							
				$trace_field_id = $field_id;											// normal field
			else
				$trace_field_id = substr($field_id,0,8).'-'.substr($field_id,8);		// multiple checkbox
...
// now the field type specific validation
		...
		switch ($field->field_type)
			{
			case LAFC_FIELD_SUBJECT:
				$bad_subject_chars = "|<>`";								// characters we don't allow

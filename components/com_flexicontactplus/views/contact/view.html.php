...
// Placeholder
	...
	switch ($field->field_type)
		...
		case LAFC_FIELD_ADVANCED:
			...
			$html .= "\n".'<input type="text" '.$field_class.$size.' name="'.$field_id.'" id="'.$field_id.'"'.$placeholder. 
		...
		case LAFC_FIELD_PASSWORD:
			...
			$html .= self::draw_label($field);
			$size = self::size($field->width);
			$html .= "\n".'<input type="password" '.$field_class.$size.' name="'.$field_id.'" id="'.$field_id.'"'.$placeholder.
				' value="'.$data->$field_id.'"'.$title.' />';
			$html .= $info_button;
			$html .= '<span id="'.$error_id.'"></span>';
			$html .= "\n".'</div>';
...
// draw a field label
//
static function draw_label($field, $class='', $extra='')
{
	$prompt = $field->prompt;
	...
	if ($field->mandatory)
		return "\n".'<label'.$for_id.$class_string.$extra.'><span class="fcp_mandatory">'.$prompt.'</span></label>';
	else
		return "\n".'<label'.$for_id.$class_string.$extra.'>'.$prompt.'</label>';
}
...
//
static function &make_radio_list($name, $current_value, &$items, $css_class='', $extra='')
{
	$html = '';
	if ($items == null)
		return '';
	foreach ($items as $key => $value)
		...
		$html .= "\n".'<input type="radio" name="'.$name.'" id="'.$name.$key.'" value="'.$key.'" '.$checked.$extra.'/>';
		$html .= "\n".'<label for="'.$name.$key.'"'.$extra.'><span>'.$value.'</span></label>';
		$html .= "\n".'</span>';
		}
	return $html;
}
...
// $extra         : Javascript or styling to be added to the <input> and <label> tags
//
static function &make_checkbox_list($name, &$items, $extra='')
{
	$html = '';
	if ($items == null)
		return '';
	foreach ($items as $key => $value)
		...
		$html .= "\n".'<input type="checkbox" name="'.$name.$key.'" id="'.$name.$key.'" value="1"'.$extra.'/>';
		$html .= "\n".'<label for="'.$name.$key.'"'.$extra.'><span>'.$value.'</span></label>';
		$html .= "\n".'</span>';
		}
	return $html;
...
//-------------------------------------------------------------------------------
// Make Tooltip text work like MooTools in jQuery
//
static function tooltipText($text)
{
	if ($text == '')
		return '';
...
// if text contains '::' (Mootools format).
...
// Make an info button
//
static function make_info($text)
{
	...
	$html = '<span class="fcp_info hasTooltip" title="'.htmlspecialchars($text, ENT_COMPAT, 'UTF-8').'"></span>';

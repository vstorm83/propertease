...
foreach ($list as $i => &$item) :
	...
	if ($item->deeper) {
		if($item->level > $startLevel && $params->get('expand',0)) {
			...
			echo '<div class="dj-subwrap '.($subcols[$item->id] > 1 ? 'multiple_cols':'single_column').' subcols'.$subcols[$item->id].'" style="width:'.$subwidth[$item->id].'px">';
	...
	elseif ($item->shallower) {
		...
		if($item->level > $startLevel+1 && $params->get('expand',0)) {
			...
			echo str_repeat('</ul></div><div style="clear:both;height:0"></div></div></li>', $item->level_diff);
		}
	}

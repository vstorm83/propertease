...
// 			$info = 
	...
	public function profilegetoutput() {
		if(intval($_REQUEST['concr'])) {
			...
			return '<div class="alert alert-danger expiredPanel" role="alert">Your subscription is either pending or expired. '.
			'<a href="index.php/plans.html">Click here to renew or upgrade your subscription.</a></div>'.
			'<script type="text/javascript">jQuery(document).ready(function($){$(".reportCreator").show();});</script>';
		}
	}
...
<p id="refw" style="display:none;">
	...
	<input name="reference" type="text" value="Untitled">
</p>
...
// 							$or=true;
	...
	<div class="ptlogo">
		<div class="pti">';
			...
			$ht.='<table style="border:none;border-collapse:collapse"  border="0" cellpadding="6" cellspacing="0" width="100%"><tbody>
	<tr align="left" valign="middle">
		<td  align="left">';
		}
		$ht.='<img src="'.JURI::base().'templates/'.$app->getTemplate().'/images/report-page-1-02.jpg" alt="Propertease" width="200px">';
		if(!$simple) {
			$ht.='</div>
	</div>
	<div class="pthinfo">
		...
		<div class="pthinfot">';
		   ...
		   $ht.='<div style="margin: 20px 0 15px 0">
						<p><span style="color:#7ab700">Report for:</span>'.(empty($search['by'])?'N/A':(profilef::hte($search['by']))).'</p>
						<p><span style="color:#7ab700">Date:</span>'.(empty($search['date'])?'N/A':(profilef::hte($search['date']))).'</p>
						<p><span style="color:#7ab700">Reference:</span>'.(empty($search['reference'])?'N/A':(profilef::hte($search['reference']))).'</p>
						</div></td>';			
		}
		if(!$simple) {
		$ht.='<table '.($simple?'cellspacing="0" cellpadding="6" border="0"':'').'>
			...
			$ht.='<td align="right" style="font-size: 11px;text-align: left;width: 50%">
			<div style="margin-top:100px;font-size: 11px;color:#7e7e7e;line-heigh: 100%">
					<br><br><br><br>
                        <div style="margin:0px 0;font-size: 11px;line-height:100%;">Propertease Pty Ltd,</div>
                         <div style="margin:0px 0;font-size: 11px;line-height:100%;">Phone 0738448296 </div>
                        <div style="margin:0px 0;font-size: 11px;line-height:100%;">Po Box 153 , Read Hill Qld 4059</div>
                         <div style="margin:0px 0;font-size: 11px;line-height:100%;">info@propertease.com.au - www.propertease.com.au</div>
                    </div>
					</td>';
			
			$ht.='</tr>
                <tr align="left">
                    <th colspan="2" bgcolor="#eaeaea" style="background:#eaeaea;border-bottom: 2px solid #7AB700;padding: 15px;width:100%">OVERVIEW</th>
                </tr>
            </tbody>
        </table>';
			
			$ht.='<table border="0" style="border:none;border-collapse:collapse" cellpadding="6" cellspacing="0" width="100%">
            <tbody>
                <tr align="left" valign="middle">
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px ">Local Council</td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " colspan="2">'.(empty($data['council'])?'N/A':(profilef::hte($data['council']))).'</td>
                </tr>
                <tr align="left" valign="middle">
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px ">Planning Scheme </td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " colspan="2">'.(empty($data['scheme'])?'N/A':(profilef::hte($data['scheme']))).'</td>
                </tr>
                <tr align="left" valign="middle">
                ...
                <tr align="left" valign="middle">
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >Overlays</td>
                    <td  style="border-bottom:  2px solid #eaeaea;padding: 15px " colspan="2">';
					 $overlaynames=array();
                ...
                <tr align="left" valign="middle">
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >Neighbourhood Plan <sup>1</sup> + Precinct</td>
                    <td  style="border-bottom:  2px solid #eaeaea;padding: 15px " colspan="2">'.(empty($data['precinct'])?'':(profilef::hte($data['precinct']))).'</td>
                </tr>
                <tr align="left" valign="middle">
                    <th colspan="3" bgcolor="#eaeaea" style="background:#eaeaea;border-bottom: 2px solid #7AB700;padding: 15px">RECONFIGURATION OF A LOT (e.g. Subdivision)</th>
                </tr>
                <tr align="left" valign="middle">
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >Minimum Lot Size and Frontage Required'.($data['minfromplan']?' <sup>2</sup>':'').'</td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >'.(is_null($data['min'])?(is_null($data['minfrontage'])?'N/A':$data['minfrontage'].'m frontage'):(is_null($data['minfrontage'])?$data['min'].'sqm':$data['min'].'sqm and '.$data['minfrontage'].'m frontage')).'</td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >'.nl2br(profilef::hte((!empty($data['lotinfo'])?$data['lotinfo']:'N/A'))).'</td>
                </tr>
                <tr align="left" valign="middle">
                    <th colspan="3" bgcolor="#eaeaea" style="background:#eaeaea;border-bottom: 2px solid #7AB700;padding: 15px">MULTIPLE DWELLING ('.(!empty($data['muddefinition'])?profilef::hte($data['muddefinition']).') '.(!empty($data['muddisclaimer'])?'<sup>3</sup>':''):'Any multiple residential development)').'</th>
                </tr>
                <tr align="left" valign="middle">
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >Minimum Lot Size and Frontage Required for unit development'.($data['minmultiplefromplan']?' <sup>2</sup>':'').'</td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >'.(is_null($data['minmultiple'])?(is_null($data['minmultiplefrontage'])?'N/A':$data['minmultiplefrontage'].'m frontage'):(is_null($data['minmultiplefrontage'])?$data['minmultiple'].'sqm':$data['minmultiple'].'sqm and '.$data['minmultiplefrontage'].'m frontage')).'</td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >'.nl2br(profilef::hte((!empty($data['lotmultipleinfo'])?$data['lotmultipleinfo']:'N/A'))).'</td>
                </tr>
                <tr align="left" valign="middle">
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >Maximum Height Prescribed'.($data['maxheightfromplan']?' <sup>2</sup>':'').'</td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >'.(is_null($data['maxstorey'])?(is_null($data['maxheight'])?'N/A':$data['maxheight'].'m above Natural Ground Level'):(is_null($data['maxheight'])?$data['maxstorey'].' storeys':$data['maxstorey'].' storeys and '.$data['maxheight'].'m above Natural Ground Level')).'</td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >'.nl2br(profilef::hte((!empty($data['heightinfo'])?$data['heightinfo']:'N/A'))).'</td>
                </tr>
                <tr align="left" valign="middle">
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >Maximum Gross Floor Area/Plot Ratio Prescribed'.($data['maxgfafromplan']?' <sup>2</sup>':'').'</td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >'.(is_null($data['maxgfa'])?'N/A':$data['maxgfa'].'% of the total site area').'</td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >'.nl2br(profilef::hte((!empty($data['gfainfo'])?$data['gfainfo']:'N/A'))).'</td>
                </tr>
                <tr align="left" valign="middle">
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >Maximum Site Cover Prescribed'.($data['maxcoverfromplan']?' <sup>2</sup>':'').'</td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >'.(is_null($data['maxcover'])?'N/A':$data['maxcover'].'% of the total site area').'</td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >'.nl2br(profilef::hte((!empty($data['coverinfo'])?$data['coverinfo']:'N/A'))).'</td>
                </tr>
                <tr align="left" valign="middle">
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >Maximum Density Prescribed'.($data['maxdensityfromplan']?' <sup>2</sup>':'').'</td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >'.(is_null($data['maxdensity'])?'N/A':$data['maxdensity'].' per hectare').'</td>
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >'.nl2br(profilef::hte((!empty($data['densityinfo'])?$data['densityinfo']:'N/A'))).'</td>
                </tr>';
					 if(count($data['overlays'])>0) {
					 	$ht.='
						  <tr align="left" valign="middle">
                      <th colspan="3" bgcolor="#eaeaea" style="background:#eaeaea;border-bottom: 2px solid #7AB700;padding: 15px">ADDITIONAL OVERLAY INFORMATION </th>
                    </tr>';
					 	foreach($data['overlays'] as $overlay) {
					 		$ht.='
						  <tr align="left" valign="middle">
							 <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >'.profilef::hte($overlay[0]).'</td>
		                <td  style="border-bottom:  2px solid #eaeaea;padding: 15px " colspan="2">'.nl2br(profilef::hte($overlay[1])).'</td>
		              </tr>';
					 	}
					 }
		...
		if(!empty($data['mudsetback'])||!empty($data['housesetback'])||!empty($data['smalllotsetback'])||!empty($data['secondarysetback'])) {
			$ht.='<table border="0" style="border:none;border-collapse:collapse" cellpadding="6" cellspacing="0" width="100%">
				<tbody>
                <tr align="left" valign="middle">
                    <th colspan="3" bgcolor="#eaeaea" style="background:#eaeaea;border-bottom: 2px solid #7AB700;padding: 15px">GENERAL INFORMATION (Note, any 
                        information following are general provisions that may be altered by the 
                        zoning, neighbourhood plan and/or overlays of a site)
                    </th>
                </tr>';
			
			if(!empty($data['mudsetback'])) {
				$ht.='<tr align="left" valign="middle">
				<td style="border-bottom:  2px solid #eaeaea;padding: 15px " >Prescribed setbacks for a Multiple Unit Dwelling'.($data['mudsetbackfromplan']?' <sup>2</sup>':'').'</td>
				<td style="border-bottom:  2px solid #eaeaea;padding: 15px "  colspan="2">'.nl2br(profilef::hte($data['mudsetback'])).'</td>
				</tr>';			
			}
			if(!empty($data['housesetback'])) {
				$ht.='<tr align="left" valign="middle">
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >Prescribed setbacks for a House'.($data['housesetbackfromplan']?' <sup>2</sup>':'').'</td>
                    <td  style="border-bottom:  2px solid #eaeaea;padding: 15px " colspan="2">'.nl2br(profilef::hte($data['housesetback'])).'</td>
                </tr>';
			}
			if(!empty($data['smalllotsetback'])) {
				$ht.='<tr align="left" valign="middle">
                    <td style="border-bottom:  2px solid #eaeaea;padding: 15px " >Prescribed setbacks for Small Lot House (450sqm or less)'.($data['smalllotsetbackfromplan']?' <sup>2</sup>':'').'</td>
                    <td  style="border-bottom:  2px solid #eaeaea;padding: 15px " colspan="2">'.nl2br(profilef::hte($data['smalllotsetback'])).'</td>
                </tr>';				
			}
			if(!empty($data['secondarysetback'])) {
				$ht.='<tr align="left" valign="middle">
                    <td style="border-bottom:  0px solid #eaeaea;padding: 15px " >Prescribed secondary Dwelling Provisions'.($data['secondarysetbackfromplan']?' <sup>2</sup>':'').'</td>
                    <td style="border-bottom:  0px solid #eaeaea;padding: 15px " colspan="2">'.nl2br(profilef::hte($data['secondarysetback'])).'</td>
                </tr>';			
			}
			$ht.='</tbody>
        </table>
		...
		$ht.='<table bgcolor="#7ab700" style="background:#7ab700;color:white" border="0" cellpadding="6" cellspacing="0" width="100%">
            <tbody>
                <tr>
                    <td style="padding: 40px 20px">
					<br><br>
                        <sup>1</sup> Neighbourhood Plan can refer to a Local Plan, Precinct, Local Area... etc. dependent on the planning scheme.<br><br>
                        <sup>2</sup> Provision taken from a Local or Neighbourhood Plan or Precinct.<br><br>
                        '.(!empty($data['muddisclaimer'])?'<sup>3</sup> '.profilef::hte($data['muddisclaimer']).'<br><br>':'').'
                        Please refer to the local planning scheme for definitions of 
                        administrative definitions (e.g. Gross Floor Area, Plot Ratio, Site 
                        Cover... etc.).<br><br>
                        We further note the following acronyms may be used throughout this 
                        website: GFA (Gross Floor Area), MCU (Material Change Of Use), SQM 
                        (Square Metres).
			<br><br>
                    </td>
                </tr>
		...
		$ht.='<div>Propertease Pty Ltd ACN 603 689 256 as trustee for the Propertease Trust  own 
				and control all the copyright and other intellectual property rights in our website 
				at http://propertease.com.au/ (“Website”) and all material contained on and accessed through our website.  
				All the copyright and other intellectual property rights in our website and the material on our website are reserved 2015.  
				You must not republish, sell, rent or sub-licence, distribute or 
				transmit any material from our website (including republication on another website).  
				The information contained on the website and any reports processed through 
				the website has been obtained from local government websites and other material provided by local government.  
				Propertease Pty Ltd as trustee for the Propertease Trust together with these local 
				governments make no representations or warranties about the reliability, completeness or suitability of 
				the data for any particular purpose and disclaims all responsibility and all liability (including without limitation, 
				liability in negligence) for all expenses, losses and damages (including indirect or consequential damage) 
				and costs which might be incurred as a result of the data being inaccurate or incomplete in any way and for any reason.  
				By utilising the Website you are agreeing to abide by and be bound by 
				the terms and conditions of use (including this disclaimer) of the Website contained <a href="http://propertease.com.au/conditions-of-use.html">here</a>.</div>';
	  }
...
*/
		...
		<h3 class="orange-arrow-down"><span class="light">Below, you can </span>review<span class="light">,</span> <i class="green">download</i> <span class="light">your</span> last report</h3>
		<div class="col-sm-12">

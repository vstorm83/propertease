...
class FsssControllerKbart extends FsssController
{
	...
	function save()
	{
		...
		if (array_key_exists('error',$file) && $file['error'] == 0)
        {
			...
			if (!file_exists(JPATH_SITE.DS.FSS_Settings::get('attach_location').DS.'kb'))
				mkdir(JPATH_SITE.DS.FSS_Settings::get('attach_location').DS.'kb', 0755, true);

            $random = 0;
			$destname = JPATH_SITE.DS.FSS_Settings::get('attach_location').DS.'kb'.DS.$random.'-'.$file['name'];    
        ...
        if (count($array) > 0)
        {
            foreach ($array as $offset => $id)
            {
               ...
               $query = 'SELECT *' .
				...
				$destname = JPATH_SITE.DS.FSS_Settings::get('attach_location').DS.'kb'.DS.$file['diskfile'];

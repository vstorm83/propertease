...
class FSSUpdater
{
	...
	function UpdatePluginsTickets()
	{
		...
		foreach ($files as $file)
		{
			...
			if (class_exists($classname))
			{
				...
				if (in_array($info['filename'], array('domaingroup', 'jomsocial', 'uddeim', 'sample', 'acysms')))
					$enabled = false;

				$log .= $this->setupPlugin("tickets", $info['filename'], $title, $description, $enabled);
	...
	function UpdatePluginsGUI()
	{
		...
		foreach ($files as $file)
		{
			...
			if (class_exists($class))
			{
				...
				if (in_array($id, array('department_tabs', 'example', 'openrelatedticket')))
					$enabled = false;

				$log .= $this->setupPlugin("gui", $id, $title, $description, $enabled);
	...
	function UpdatePluginsTicketPrint()
	{
		...
		foreach ($files as $file)
		{
			...
			$log .= $this->setupPlugin("ticketprint", $id, $title, $description, 1);
	...
	function UpdatePluginsUserList()
	{
		...
		foreach ($files as $file)
		{
			...
			$log .= $this->setupPlugin("userlist", $id, $title, $description, $enabled);
	...
	function setupPlugin($type, $name, $title, $description, $enabled)
	{
		$db = JFactory::getDBO();
		...
		if ($record)
		{
			...
			$sql = "UPDATE #__fss_plugins SET title = '" . $db->escape($title) . "', description = '" . $db->escape($description) . "'";
			...
			// create entry
			$sql = "INSERT INTO #__fss_plugins (`type`, name, title, description, enabled) VALUES (";
			$sql .= "'" . $db->escape($type) . "', ";
			$sql .= "'" . $db->escape($name) . "', ";
			$sql .= "'" . $db->escape($title) . "', ";

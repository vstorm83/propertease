...
static function validate_email_list(&$email_list, $allow_blank=true)
{
	...
	$email_list = trim($email_list,',');						// trim off any spare commas
...
static function make_title($title, $config_data=null, $config_count=0)
{
	if (($config_data == null) or ($config_count <= 1))
		JToolBarHelper::title(LAFC_COMPONENT_NAME.': '.JText::_($title), 'flexicontactplus.png');
	else
		JToolBarHelper::title(LAFC_COMPONENT_NAME.': '.'<span class="fcp_config_title">'.
			$config_data->name.' ['.$config_data->language.']</span>'.JText::_($title),'flexicontactplus.png');
			
// set the browser page title without the config name or the html

	$app = JFactory::getApplication();
	$sitename = $app->getCfg('sitename');
	$document = JFactory::getDocument();
	$document->setTitle($sitename.' - '.JText::_('JADMINISTRATION').' - '.LAFC_COMPONENT_NAME.': '.JText::_($title));
...
{
	JSubMenuHelper::addEntry(JText::_('COM_FLEXICONTACT_CONFIGURATION'), 'index.php?option='.LAFC_COMPONENT.'&controller=menu&task=display',        $submenu == 'config');
	JSubMenuHelper::addEntry(JText::_('COM_FLEXICONTACT_TOOLBAR_IMAGES'), 'index.php?option='.LAFC_COMPONENT.'&controller=menu&view=config_images', $submenu == 'images');
	JSubMenuHelper::addEntry(JText::_('COM_FLEXICONTACT_CONFIG_CSS_NAME'), 'index.php?option='.LAFC_COMPONENT.'&controller=menu&view=config_css',   $submenu == 'css');
	JSubMenuHelper::addEntry(JText::_('COM_FLEXICONTACT_LOG'), 'index.php?option='.LAFC_COMPONENT.'&controller=log&task=display',                   $submenu == 'log');
	JSubMenuHelper::addEntry(JText::_('COM_FLEXICONTACT_HELP_AND_SUPPORT'), 'index.php?option='.LAFC_COMPONENT.'&controller=menu&task=help',        $submenu == 'help');
}
...
static function make_field_type_list()
{
	...
	$field_types[LAFC_FIELD_ADVANCED]     = JText::_('COM_FLEXICONTACT_FIELD_REGEX');
	...
	$field_types["OPTGROUP_START_5"]      = JText::_('COM_FLEXICONTACT_DEPRECATED');
	$field_types[LAFC_FIELD_CHECKBOX_H]   = JText::_('COM_FLEXICONTACT_FIELD_CHECKBOX_H');
	$field_types["OPTGROUP_END_5"]          = '';
...
// Make an info button
//
static function make_info($text, $link='')
{
	...
	if (strpos($text, '::'))								// handle the old MooTools title convention
		{
		list($title, $content) = explode('::', $text, 2);	// Split into title and content 
		if ($title == '')
			$text = $content;
		else
			$text = '<strong>'.$title.'</strong><br />'.$content;
		}
	
	$html .= '<span class="hasTip" title="'.htmlspecialchars($text, ENT_COMPAT, 'UTF-8').'">'.$icon.'</span>';
...
// Check if the tmp directory is writeable

	$tmp_dir = $app->getCfg('tmp_path');
	if (!is_writable($tmp_dir))
		$app->enqueueMessage(JText::_('COM_FLEXICONTACT_TMP_DIR_UNWRITEABLE'), 'notice');
}

...
class FlexicontactplusModelLog extends LAFP_model
{
var $_data;
var $_app = null;
...
function store($email_data)
{
	...
	$query = 'INSERT INTO `#__flexicontact_plus_log` 
		(`datetime`, `name`, `email`, `admin_email`, `config_name`, `config_lang`,`subject`, `message`, `status_main`, `status_copy`, 
			`ip`, `browser_id`, `browser_string`, `attached_file`,`imported` ) 
		VALUES
			( NOW(), '.
			...
			$this->_db->Quote($email_data->attached_file).','.
			'0)';				// set imported to 0
...
// build the query
	...
	$query_cols  = "Select id, datetime, name, email, admin_email, subject, SUBSTRING(message,1,60) AS short_message, status_main, status_copy, attached_file  ";
...
function count_rows()
{
	$query = "SELECT COUNT(*) FROM `#__flexicontact_plus_log`";
...
{
	$query = "SELECT DISTINCT(config_name) FROM `#__flexicontact_plus_log` WHERE `config_name` != '' order by `config_name`";
...
//--------------------------------------------------------------------	
// Get the information for the log chart
//
...
// Set up the overall chart information
	...
	$chart_info->x_size = 0;		// size of container
...
//-------------------------------------------------------------------------------
// Export the log table as per the current filters
//
function export_csv()
{
